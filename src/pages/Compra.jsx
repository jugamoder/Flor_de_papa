...
import { db, genUUID, touchMovimiento, SENTINEL_ID, getOcasionalCorrelativo } from '../db/database';
import { ChevronLeft, Plus, X, Search, Check, Delete, Trash2, RotateCcw, Settings2, CheckCircle, DollarSign, Printer, ArrowLeftRight } from 'lucide-react';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';

const FONT = '"Inter","SF Pro Display",system-ui,sans-serif';
const BLU = '#2563eb'; const BLU_DK = '#1d4ed8';

function getLum(hex) {
  const c = (hex || '#000').replace('#', ''); if (c.length < 6) return 0;
...
function VariedadModal({ variedades, excluir = [], onSelect, onClose, uuidActivo }) {
  ...
  const [form, setForm] = useState({ nombre: '', codigo_corto: '', color: '#3B82F6' });
...
/* ─── NuevoSocioTransbordoModal ─── */
function NuevoSocioTransbordoModal({ onClose, onCreado }) {
  ...
  const handleSave = async () => {
    ...
    onCreado({ id_socio: id, nombre: form.nombre.trim(), documento: form.documento.trim(), telefono: form.telefono.trim(), es_cliente: 1 });
  };
...
function ClienteDestinoModal({ clientes, valorActual, onSelect, onClose, onNuevoSocio, onTransbordoRapido, fechaSeleccionada }) {
  const [q, setQ] = useState('');
  const [correlativo, setCorrelativo] = useState(1);

  useEffect(() => {
    getOcasionalCorrelativo('venta', fechaSeleccionada).then(n => setCorrelativo(n));
  }, [fechaSeleccionada]);

  const lista = clientes.filter(c =>
    String(c.id_socio) !== SENTINEL_ID &&
    c.nombre.toLowerCase().includes(q.toLowerCase())
  );

  const labelRapida = q.trim()
    ? `Transbordo Rápido (${q.trim()})`
    : `Transbordo Rápido (Cliente ${correlativo})`;
...
/* ─── BuscadorSocioModal ─── */
function BuscadorSocioModal({ socios, term = '', onClose, onSelect, onNuevaFrecuente, onNuevaOcasional, colorTema = 'blue', tipoRapida = 'compra', fechaSeleccionada }) {
  ...
  const th = themeColors[colorTema] || themeColors.blue;
  ...
  const labelRapida = q.trim()
    ...
    : (esCompra ? `Compra Rápida (Proveedor ${correlativo})` : `Venta Rápida (Cliente ${correlativo})`);
...
function PreLiquidadorModal({
  ...
  const fleteMontoTotalLocal = useMemo(() => {
    const precio = parseFloat(fletePrecioKg) || 0;
    return totalPesoNeto * precio;
  }, [totalPesoNeto, fletePrecioKg]);

  const fleteNetoLocal = fleteActivo === 1 ? (fleteTipoSigno === 'RESTAR' ? -fleteMontoTotalLocal : fleteMontoTotalLocal) : 0;

  const montoFinalLocal = useMemo(() => {
    const base = totalDineroLocal - totalExtras + totalAdicionales;
    if (tipoMov === 'compra') {
      return base + fleteNetoLocal;
    }
    return base;
  ...
  const labelPago = isCompra ? 'REGISTRAR PAGO' : 'REGISTRAR COBRO';
...
export default function Compra() {
  ...
  const [uuidActivo, setUuidActivo] = useState(() => localStorage.getItem('compra_activa_uuid') || '');
  const [proveedorId, setProveedorId] = useState('');
  const [fecha, setFecha] = useState(() => {
    const activeUuid = localStorage.getItem('compra_activa_uuid') || '';
    return activeUuid ? '' : hoy;
  });
  const [precios, setPrecios] = useState({});
  const [modoTransbordo, setModoT] = useState(false);
  const [clienteDestino, setCD] = useState(null);
  const [uuidVentaTB, setUuidVentaTB] = useState(null);

  const [variedadesSeleccionadas, setVarSel] = useState([]);
  const [variedadActiva, setVarA] = useState(null);
  const [modoConfig, setModoC] = useState(false);
  const [showVM, setShowVM] = useState(false);
  const [swapTarget, setST] = useState(null);

  const [peso, setPeso] = useState('');
  const [flashOk, setFlashOk] = useState(false);
  const [showGastos, setSG] = useState(false);
  const [editSaco, setES] = useState(null);
  const [confirmDel, setCD2] = useState(null);   // id_saco a eliminar
  ...
  const [showNuevoSocioTB, setShowNuevoSocioTB] = useState(false);
  const [showBuscadorProv, setShowBP] = useState(false);
  ...
  // Flete automático
  const [fletePrecioKg, setFletePrecioKg] = useState('');
  const [fleteTipoSigno, setFleteTipoSigno] = useState('RESTAR');
  const [fleteActivo, setFleteActivo] = useState(0);

  const proveedores = useLiveQuery(() => db.socios.where('es_proveedor').equals(1).toArray()) || [];
  ...
  // Carrusel: compras activas de hoy (incluye la activa aunque sea de fecha pasada para que no desaparezca de la UI)
  const comprasHoy = useLiveQuery(async () => {
    const hoyMovs = await db.movimientos.where('fecha').equals(hoy).filter(m => m.tipo === 'compra' && m.estado !== 'finalizado').toArray();
    if (uuidActivo) {
      const active = await db.movimientos.where('uuid').equals(uuidActivo).first();
      if (active && active.fecha !== hoy && active.estado !== 'finalizado' && active.tipo === 'compra') {
  ...
  const sacosResumen = useLiveQuery(async () => {
    if (!comprasHoy.length) return {};
    const uuids = comprasHoy.map(v => v.uuid);
    const todosSacos = await db.sacos.where('uuid_movimiento').anyOf(uuids).toArray();
    const res = {};
  ...
  }, [comprasHoy, sacosResumen, uuidActivo]);
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      ...
      setProveedorId(mov.id_socio ? String(mov.id_socio) : '');
      if (mov.fecha) setFecha(mov.fecha);
      if (mov.precios_por_variedad) setPrecios(mov.precios_por_variedad);

      // Hydrate flete
      if (mov.flete_activo !== undefined) {
        setFleteActivo(mov.flete_activo);
      } else {
        setFleteActivo(0);
      }
      if (mov.flete_precio_kg !== undefined && mov.flete_precio_kg !== null) {
        setFletePrecioKg(mov.flete_precio_kg === 0 ? '' : String(mov.flete_precio_kg));
      } else {
        setFletePrecioKg('');
      }
      if (mov.flete_tipo_signo) {
        setFleteTipoSigno(mov.flete_tipo_signo);
      } else {
        setFleteTipoSigno('RESTAR');
      }
      ...
      if (vars.length > 0) {
        setVarSel(vars);
        setVarA(nextVar || vars[0]);
      } else {
        setVarSel(nextVar ? [nextVar] : []);
        setVarA(nextVar || null);
      }

      // Check if it is a transbordo
      if (mov.modo_transbordo_activo === 1) {
        setModoT(true);
        // Find all Venta sacos originating from these Purchase sacos
        const transbordoSacos = sx.filter(s => s.es_transbordo === 1);
        const sacosIds = transbordoSacos.map(s => s.id_saco);
        const vSacos = await db.sacos.where('id_saco_origen').anyOf(sacosIds).toArray();
        const sortedVSacos = vSacos.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
        
        if (sortedVSacos.length > 0) {
          const lastVSaco = sortedVSacos[0];
          const lastVentaUuid = lastVSaco.uuid_movimiento;
          
          setUuidVentaTB(lastVentaUuid);
          localStorage.setItem('transbordo_activo_uuid', lastVentaUuid);
          
          // Get the Venta movement to determine the client and name
          const vMov = await db.movimientos.where('uuid').equals(lastVentaUuid).first();
          if (vMov) {
            if (vMov.id_socio === SENTINEL_ID) {
              setCD({
                id_socio: SENTINEL_ID,
                nombre: vMov.socio_nombre_temporal || 'Transbordo Rápido',
                es_ocasional: 0
              });
            } else {
              const cliente = await db.socios.get(vMov.id_socio);
              if (cliente) {
                setCD(cliente);
              }
            }
        ...
        setCD(null);
        setUuidVentaTB(null);
        localStorage.removeItem('transbordo_activo_uuid');
      }
    }
    hydrate();
  ...
  const handleApplyAjuste = useCallback(async (vid, tipo_ajuste, kilos_ajuste) => {
    ...
    if (modoTransbordo && uuidVentaTB) {
      const vMov = await db.movimientos.where('uuid').equals(uuidVentaTB).first();
      if (vMov) {
        const vVars = vMov.variedades || {};
        const vVar = vVars[vid] || { peso_bruto: bruto, tipo_ajuste: 'NINGUNO', kilos_ajuste: 0, peso_neto: bruto, unidad_precio: 'KG' };
        let vNeto = bruto;
        if (tipo_ajuste === 'RESTAR') {
          vNeto = Math.max(0, bruto - kilos_ajuste);
        } else if (tipo_ajuste === 'SUMAR') {
          vNeto = bruto + kilos_ajuste;
        }
        await db.movimientos.where('uuid').equals(uuidVentaTB).modify({
          variedades: {
            ...vVars,
            [vid]: {
              ...vVar,
  ...
  }, [uuidActivo, activeMov, modoTransbordo, uuidVentaTB]);
  ...
  const toggleRowUnit = useCallback(async (vid) => {
    ...
    if (modoTransbordo && uuidVentaTB) {
      ...
      if (vMov) {
        const vVars = vMov.variedades || {};
        const vVar = vVars[vid] || {
          peso_bruto: currentVar.peso_bruto,
          tipo_ajuste: 'NINGUNO',
          kilos_ajuste: 0,
          peso_neto: currentVar.peso_neto,
          unidad_precio: 'KG'
        };
        await db.movimientos.where('uuid').equals(uuidVentaTB).modify({
          variedades: {
  ...
  const handleToggleTransbordo = useCallback(async () => {
    if (!uuidActivo) return;
    const nextVal = modoTransbordo ? 0 : 1;
    setModoT(!modoTransbordo);
    await db.movimientos.where('uuid').equals(uuidActivo).modify({
      modo_transbordo_activo: nextVal,
      fecha_actualizacion: Date.now()
    });
  }, [uuidActivo, modoTransbordo]);

  // Transbordo: limpiar al desactivar
  useEffect(() => {
    if (!modoTransbordo) {
      setCD(null);
      setUuidVentaTB(null);
  ...
  useEffect(() => {
    ...
    async function updateCorrelativo() {
      ...
      // If there is an active transbordo sale, sync its date and timestamp too
      if (uuidVentaTB) {
        const tbMov = await db.movimientos.where('uuid').equals(uuidVentaTB).first();
        if (tbMov && tbMov.fecha !== fecha && active) {
          await db.movimientos.where('uuid').equals(uuidVentaTB).modify({
            fecha,
            timestamp: combinedTime + 1,
            fecha_actualizacion: combinedTime + 1
          });
        }
      }
      ...
      if (mov.id_socio === SENTINEL_ID) {
        ...
        const isDefault = !currentName || currentName === 'Compra Rápida';

        if (isDefault || dateChanged) {
          const correlativo = await getOcasionalCorrelativo('compra', fecha);
          const newName = `Compra Rápida (Proveedor ${correlativo})`;
      ...
      // Handle Transbordo Rápido client name reactivity
      if (modoTransbordo && clienteDestino && clienteDestino.id_socio === SENTINEL_ID) {
        const currentDestName = clienteDestino.nombre || '';
        const isDestDefault = !currentDestName || currentDestName === 'Transbordo Rápido';

        if (isDestDefault || dateChanged) {
          const destCorrelativo = await getOcasionalCorrelativo('venta', fecha);
          const newDestName = `Transbordo Rápido (Cliente ${destCorrelativo})`;
          if (newDestName !== currentDestName && active) {
            setCD(prev => prev ? { ...prev, nombre: newDestName } : null);
            if (uuidVentaTB) {
              await db.movimientos.where('uuid').equals(uuidVentaTB).modify({
                socio_nombre_temporal: newDestName,
                fecha_actualizacion: Date.now()
              });
            }
          }
    ...
    updateCorrelativo();
    return () => { active = false; };
  }, [fecha, uuidActivo, uuidVentaTB, modoTransbordo, clienteDestino?.id_socio]);

  const handleSelectCD = useCallback(async (cliente) => {
    setCD(cliente);
    const ventasHoy = await db.movimientos.where('fecha').equals(fecha).filter(m => m.tipo === 'venta' && m.id_socio === cliente.id_socio && m.estado !== 'finalizado').toArray();
    ...
    if (ventasHoy.length) {
      setUuidVentaTB(ventasHoy[0].uuid);
      localStorage.setItem('transbordo_activo_uuid', ventasHoy[0].uuid);
      await db.movimientos.where('uuid').equals(ventasHoy[0].uuid).modify({
        es_transbordo: 1,
        timestamp: combinedTime + 1,
        fecha_actualizacion: combinedTime + 1
      });
      ...
      await db.movimientos.add({
        ...
        timestamp: combinedTime + 1,
        fecha_actualizacion: combinedTime + 1,
        id_socio: cliente.id_socio,
        es_transbordo: 1
      });
      setUuidVentaTB(nu);
      localStorage.setItem('transbordo_activo_uuid', nu);
    }
  }, [fecha]);

  const handleSelectTransbordoRapido = useCallback(async (nombreProv) => {
    const correlativo = await getOcasionalCorrelativo('venta', fecha);
    const nombreTemporal = nombreProv
      ? `Transbordo Rápido (${nombreProv})`
      : `Transbordo Rápido (Cliente ${correlativo})`;
  ...
  // Computed
  const sacosDeVar = variedadActiva ? sacos.filter(s => s.id_variedad === variedadActiva.id_variedad) : [];
  const totalV = sacosDeVar.length;
  const loteActual = totalV % 5 === 0 && totalV > 0 ? sacosDeVar.slice(-5) : sacosDeVar.slice(-(totalV % 5 || 0));
  const loteNum = Math.floor(totalV / 5) + (totalV % 5 === 0 ? 0 : 1);
  const loteSuma = loteActual.reduce((s, x) => s + x.peso, 0);
  const accent = variedadActiva?.color || BLU;

  const varMap = useMemo(() => Object.fromEntries(variedades.map(v => [v.id_variedad, v])), [variedades]);
  const varIds = useMemo(() => [...new Set(sacos.map(s => s.id_variedad))], [sacos]);
  const cntPV = useMemo(() => { const c = {}; sacos.forEach(s => { c[s.id_variedad] = (c[s.id_variedad] || 0) + 1; }); return c; }, [sacos]);
  ...
  }, [sacos, activeMov, precios, varIds]);
  ...
  // Flete calculation
  const fleteMontoTotal = useMemo(() => {
    const precio = parseFloat(fletePrecioKg) || 0;
    return totalPesoNeto * precio;
  }, [totalPesoNeto, fletePrecioKg]);

  const totalNeto = useMemo(() => {
    const fleteNeto = fleteActivo === 1 ? (fleteTipoSigno === 'RESTAR' ? -fleteMontoTotal : fleteMontoTotal) : 0;
    return totalDinero - totalExtras + totalAdicionales + fleteNeto;
  }, [totalDinero, totalExtras, totalAdicionales, fleteActivo, fleteTipoSigno, fleteMontoTotal]);

  const handleFletePrecioChange = useCallback(async (val) => {
    setFletePrecioKg(val);
    if (uuidActivo) {
      const precioNum = parseFloat(val) || 0;
      const montoTotal = totalPesoNeto * precioNum;
      await db.movimientos.where('uuid').equals(uuidActivo).modify({
        flete_precio_kg: precioNum,
        flete_monto_total: montoTotal
      });
    }
  }, [uuidActivo, totalPesoNeto]);

  const handleApplyFlete = useCallback(async (tipo) => {
    setFleteTipoSigno(tipo);
    setFleteActivo(1);
    if (uuidActivo) {
      const precioNum = parseFloat(fletePrecioKg) || 0;
      const montoTotal = totalPesoNeto * precioNum;
      await db.movimientos.where('uuid').equals(uuidActivo).modify({
        flete_activo: 1,
        flete_tipo_signo: tipo,
        flete_precio_kg: precioNum,
        flete_monto_total: montoTotal,
        fecha_actualizacion: Date.now()
      });
    }
  }, [uuidActivo, fletePrecioKg, totalPesoNeto]);

  const handleRemoveFlete = useCallback(async () => {
    setFleteActivo(0);
    if (uuidActivo) {
      await db.movimientos.where('uuid').equals(uuidActivo).modify({
        flete_activo: 0,
  ...
  // Sync flete total on scale/weight changes
  useEffect(() => {
    if (!uuidActivo) return;
    const precio = parseFloat(fletePrecioKg) || 0;
    const montoTotal = totalPesoNeto * precio;
    if (fleteActivo === 1) {
      db.movimientos.where('uuid').equals(uuidActivo).modify({
        flete_monto_total: montoTotal
      });
    }
  }, [totalPesoNeto, fletePrecioKg, uuidActivo, fleteActivo]);
  ...
  // Confirmar adicional (llamado por botón + o -)
  const commitAdic = (op) => {
  ...
  const switchCompra = useCallback((uuid) => {
  ...
  /* ─── Crear nueva compra (slot) ─── */
  const crearNuevaCompra = useCallback(async () => {
    const uuid = genUUID();
    // Auto-asignar Compra Rápida por defecto (sin socio = compra rápida)
  ...
  useEffect(() => { const h = () => setTimeout(refocus, 50); document.addEventListener('focusout', h); return () => document.removeEventListener('focusout', h); }, [refocus]);
  ...
  const key = useCallback((k) => {
    ...
    if (k === 'ENTER') {
      ...
      const ts = Date.now();
      db.sacos.add({ uuid_movimiento: uuidActivo, id_movimiento: uuidActivo, id_variedad: variedadActiva.id_variedad, peso: w, es_transbordo: modoTransbordo ? 1 : 0, cliente_destino_id: modoTransbordo && clienteDestino ? clienteDestino.id_socio : null, timestamp: ts })
        .then(idSaco => {
          touchMovimiento(uuidActivo);
          if (modoTransbordo && uuidVentaTB) {
            db.sacos.add({ uuid_movimiento: uuidVentaTB, id_movimiento: uuidVentaTB, id_variedad: variedadActiva.id_variedad, peso: w, es_transbordo: 1, id_saco_origen: idSaco, timestamp: ts + 1 })
              .then(() => touchMovimiento(uuidVentaTB));
          }
        });
      setPeso(''); setFlashOk(true); setTimeout(() => setFlashOk(false), 350);
      return;
    }
    if (k === '.' && peso.includes('.')) return;
    if (peso.replace('.', '').length >= 5) return;
    setPeso(p => p + k);
  }, [peso, variedadActiva, uuidActivo, modoTransbordo, clienteDestino, uuidVentaTB]);
  ...
  const addVar = v => {
    if (swapTarget != null) {
      setVarSel(vs => vs.map(x => x.id_variedad === swapTarget ? v : x));
      if (variedadActiva?.id_variedad === swapTarget) changeVarA(v);
      setST(null);
    } else {
      setVarSel(vs => { if (!vs.find(x => x.id_variedad === v.id_variedad)) return [...vs, v]; return vs; });
      changeVarA(v);
    }
  };
  const remVar = async (id) => {
    if (cntPV[id] && cntPV[id] > 0) return; // No permitir borrar si ya tiene sacos

    setVarSel(vs => {
      const novs = vs.filter(v => v.id_variedad !== id);
      if (variedadActiva?.id_variedad === id) {
        changeVarA(novs[0] || null);
      }
      return novs;
    });
  ...
  const handleProvChange = async (val) => {
    setProveedorId(val);
    if (uuidActivo && val) {
  ...
  const finalizarConLiquidacion = async (preciosFinales, montoNetoFinal, pagoMetodo, pagoMonto) => {
    if (uuidActivo) {
      ...
      if (modoTransbordo && uuidVentaTB) {
        const existingTb = await db.movimientos.where('uuid').equals(uuidVentaTB).first();
        const tbFecha = existingTb?.fecha || finalFecha;
        const tbTimestamp = existingTb?.timestamp || (finalTimestamp + 1);
        ...
        if (wasFinalizado) {
          tbUpdate.fecha_edicion = new Date().toISOString();
        }

        await db.movimientos.where('uuid').equals(uuidVentaTB).modify(tbUpdate);
      }
  ...
  const generarPDF = async () => {
    ...
    // Elimina tildes y caracteres no-ASCII que corrompen jsPDF con fuentes estandar
    const ascii = s =>
      (s || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^\x20-\x7E]/g, '');

    // Formato condicional: enteros sin decimal, decimales con 1 cifra
    const fmt = n => Number.isInteger(n) ? String(n) : n.toFixed(1);

    // Datos del proveedor
    const proveedor = proveedores.find(p => String(p.id_socio) === String(proveedorId));
    const nombreSocio = ascii(proveedor?.nombre || 'Sin asignar');
    ...
    if (config?.nombre) {
      ...
      doc.setFontSize(10);
      doc.setTextColor(37, 99, 235); // Blue color for Compra
    ...
    doc.text('LIQUIDACIÓN DE COMPRA', MARGEN, 29);

    // 2. Metadatos del Ticket (1 línea destacada sin fondo pesado)
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(0, 0, 0);
    doc.text(`PROVEEDOR: ${nombreSocio}`, MARGEN, 37);
    ...
    if (fleteActivo === 1 && fleteMontoTotal > 0) {
      const fleteSignoChar = fleteTipoSigno === 'RESTAR' ? '-' : '+';
      resumenRows.push([
        `Flete: ${fmt(totalPesoNeto)} kg x S/ ${parseFloat(fletePrecioKg).toFixed(2)}`,
        '—',
        '—',
        '—',
        `${fleteSignoChar} S/ ${fleteMontoTotal.toFixed(2)}`
      ]);
    }
    ...
    doc.save(`Compra_${hoy}_${nombreSocio.replace(/\s+/g, '_')}.pdf`);
  };
  ...
  return (
    <div className="h-[100dvh] flex flex-col overflow-hidden" style={{ background: 'var(--surface-base)', color: 'var(--text-primary)', fontFamily: FONT }}>
      ...
      {/* Re-added FinalizarModal below BuscadorSocioModal conceptually, but rendering in-line here */}
      {showFin && (
        <PreLiquidadorModal
          sacos={sacos}
          activeMov={activeMov}
          ...
          fleteActivo={fleteActivo}
          fleteTipoSigno={fleteTipoSigno}
          fletePrecioKg={fletePrecioKg}
      ...
      {showVM && <VariedadModal variedades={variedades} uuidActivo={uuidActivo} excluir={swapTarget != null ? variedadesSeleccionadas.filter(v => v.id_variedad !== swapTarget).map(v => v.id_variedad) : variedadesSeleccionadas.map(v => v.id_variedad)} onSelect={addVar} onClose={() => { setShowVM(false); setST(null); }} />}
      {showAjusteModal && (
        <AjustePesoModal
      ...
      {showCDModal && (
        <ClienteDestinoModal
          clientes={clientes}
          valorActual={clienteDestino?.id_socio}
          onSelect={handleSelectCD}
          onTransbordoRapido={handleSelectTransbordoRapido}
          onClose={() => setShowCDModal(false)}
          onNuevoSocio={() => { setShowCDModal(false); setShowNuevoSocioTB(true); }}
          fechaSeleccionada={fecha}
        />
      )}
      {showNuevoSocioTB && (
        <NuevoSocioTransbordoModal
          onClose={() => setShowNuevoSocioTB(false)}
          onCreado={(socio) => {
            setShowNuevoSocioTB(false);
            handleSelectCD(socio);
          }}
        />
      )}
      ...
      {showBuscadorProv && (
        <BuscadorSocioModal
          ...
          onSelect={c => {
            handleProvChange(c.id_socio);
            setShowBP(false);
          }}
          onNuevaFrecuente={async (data) => {
            // Crea un socio frecuente real en la tabla socios
            const id = await db.socios.add({ nombre: data.nombre.trim(), documento: data.documento.trim(), telefono: data.telefono.trim(), es_proveedor: 1, es_cliente: 0, es_ocasional: 0, saldo_actual: 0 });
            handleProvChange(id);
            setShowBP(false);
          }}
          tipoRapida="compra"
          ...
          onNuevaOcasional={async (nombre) => {
            ...
            const nombreTemporal = n
              ? `Compra Rápida (${n})`
              : `Compra Rápida (Proveedor ${correlativo})`;
            if (uuidActivo) {
              await db.movimientos.where('uuid').equals(uuidActivo).modify({
                id_socio: SENTINEL_ID,
                socio_nombre_temporal: nombreTemporal,
        ...
        <div className="flex items-center gap-1.5 px-2 pt-2 pb-1 overflow-x-auto" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          ...
          {comprasCarrusel.map((v, idx) => {
            ...
            const nombre = (!v.id_socio || v.id_socio === SENTINEL_ID)
              ? (v.socio_nombre_temporal || 'CR')
                  .replace('Compra Rápida', 'CR')
                  .replace('Venta Rápida', 'VR')
              : (proveedores.find(p => p.id_socio === v.id_socio)?.nombre || `Compra ${comprasCarrusel.length - idx}`);
            ...
            const conteos = isA ? cntPV : resumen.conteoPorVar;
              ...
              {(() => {
                ...
                if (currentSocioId) {
                  return String(currentSocioId) === SENTINEL_ID
                    ? (activeMov?.socio_nombre_temporal || 'Compra Rápida')
                    : (proveedores.find(c => String(c.id_socio) === String(currentSocioId))?.nombre || 'Proveedor Seleccionado');
                }
                const idx = comprasCarrusel.findIndex(v => v.uuid === uuidActivo);
                const pos = comprasCarrusel.length - (idx >= 0 ? idx : 0);
          ...
          {variedadesSeleccionadas.map(v => {
            ...
            return (
              <div key={v.id_variedad} className="flex-shrink-0 flex items-center rounded-lg overflow-hidden" style={{ border: isA ? `2px solid ${v.color}` : '2px solid transparent' }}>
                ...
                {modoConfig && <button onClick={() => remVar(v.id_variedad)} className="px-1 py-1.5" style={{ backgroundColor: `${v.color}22` }}><X size={11} className="text-red-400" /></button>}
              </div>
            );
          })}
